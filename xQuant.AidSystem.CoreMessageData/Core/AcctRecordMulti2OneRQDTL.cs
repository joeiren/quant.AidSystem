...
{
    public class AcctRecordMulti2OneRQDTL : IMessageReqHandler
    {
         ...
         public UInt32 TOTAL_WIDTH
        {
            get 
            {
                return (UInt32)(22 + BGR33CHPG1 * AccountingEntryMulti2One.TOTAL_WIDTH + BGR33CHPG2 * AccountingEliminateMulti2One.TOTAL_WIDTH);
            }
        }
        /// <summary>
        /// 资金业务系统流水号,18
        ...
        public List<AccountingEntryMulti2One> EntryList
        {
            get;
            set;
        }
        public List<AccountingEliminateMulti2One> EliminateList
        {
            get;
            set;
        }

        public AcctRecordMulti2OneRQDTL()
        {
            EntryList = new List<AccountingEntryMulti2One>();
            EliminateList = new List<AccountingEliminateMulti2One>();
        }
        ...
        public byte[] ToBytes()
        {
            ...
            if (BGR33CHPG1 > 0 && BGR33CHPG1 == EntryList.Count)
            {
                ...
                foreach (AccountingEntryMulti2One entry in EntryList)
                {
                    byte[] buffer = entry.ToBytes();
                    Array.Copy(buffer, 0, byteBuffer, offset, AccountingEntryMulti2One.TOTAL_WIDTH);
                    offset += AccountingEntryMulti2One.TOTAL_WIDTH;
                }
            }
            ...
            {
                foreach (AccountingEliminateMulti2One item in EliminateList)
                {
                    byte[] buffer = item.ToBytes();
                    Array.Copy(buffer, 0, byteBuffer, offset, AccountingEliminateMulti2One.TOTAL_WIDTH);
                    offset += AccountingEliminateMulti2One.TOTAL_WIDTH;
                }
            }
            return byteBuffer;
        }
    ...
    /// 会计分录
    /// </summary>
    public class AccountingEntryMulti2One : IMessageReqHandler
    {
        public const UInt16 TOTAL_WIDTH = 302;
        #region Property
        /// <summary>
        /// 套内序号,2位
        /// </summary>
        ...
        public byte[] ToBytes()
        {
            ...
            sb = sb.Append(CommonDataHelper.FillSpecifyWidthString(BGR33AC201, 22));
            CommonDataHelper.ResetByteBuffer(sb, ref bytes, ref totalLen, true);
            sb.Remove(0, sb.Length);
            ...
            CommonDataHelper.ResetByteBuffer(sb, ref bytes, ref totalLen, true);
            sb.Remove(0, sb.Length);
            sb = sb.Append(CommonDataHelper.FillSpecifyWidthString(BGR34CHSX1, 1));
            CommonDataHelper.ResetByteBuffer(sb, ref bytes, ref totalLen, true);
            sb.Remove(0, sb.Length);
    ...
    /// 挂销账
    /// </summary>
    public class AccountingEliminateMulti2One : IMessageReqHandler
    {
